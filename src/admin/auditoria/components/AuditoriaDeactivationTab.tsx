...
interface AuditUserDeactivation {
    id: number;
    deactivatedUserId: number;
    deactivatedUserEmail: string;
    deactivatedUserRole: string;
    adminUserId: number;
    adminEmail: string;
    ipAddress: string;
...
export function AuditoriaDeactivationTab() {
    const [logs, setLogs] = useState<AuditUserDeactivation[]>([]);
    ...
    const fetchLogs = async () => {
        try {
            const response = await apiRequest<{ success: boolean; data: AuditUserDeactivation[] }>('/admin/audit/user-deactivation');
            if (response.success) {
                setLogs(response.data);
            }
        } catch (error) {
            console.error('Error fetching user deactivation audits:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const filteredLogs = logs.filter(log =>
        (log.deactivatedUserEmail && log.deactivatedUserEmail.toLowerCase().includes(searchTerm.toLowerCase())) ||
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center gap-4 bg-white p-4 rounded-xl border border-gray-200 shadow-sm max-w-sm">
                ...
                <div>
                    <p className="text-sm text-gray-600">Total Usuarios Desactivados</p>
            ...
            <div className="relative max-w-md">
                ...
                <Input
                    type="text"
                    placeholder="Buscar por email (desactivado o admin)..."
                    value={searchTerm}
                    ...
                    <table className="w-full">
                        <thead className="bg-gray-50 border-b border-gray-200">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Usuario Desactivado</th>
                                ...
                                filteredLogs.map((log) => (
                                    <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="text-sm text-gray-900">{new Date(log.deactivatedAt).toLocaleString('es-ES')}</div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="text-sm font-medium text-gray-900">{log.adminEmail}</div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="text-sm font-medium text-red-600">{log.deactivatedUserEmail}</div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                                                {log.deactivatedUserRole}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
